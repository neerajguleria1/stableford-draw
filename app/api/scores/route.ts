...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!score || !played_at) return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(dateOnly)) return NextResponse.json({ error: "Invalid date format" }, { status: 400 });
    if (new Date(dateOnly) > new Date()) return NextResponse.json({ error: "Date cannot be in the future" }, { status: 400 });
    ...
    if (existing) return NextResponse.json({ error: "You already have a score for this date" }, { status: 409 });
